...
function replace(req, res) {
    for (var i = replaces.length - 1; i >= 0; i--) {
        var item = replaces[i];
        var params = item.pattern.match(req.url);
        if (params) {
            req.params = params;
...
module.exports = {
    ...
    replaceFile: function(file) {
        replaces.push({
            patternStr: file.pattern,
            pattern: urlPattern.newPattern(file.pattern),
            callback: file.handler
        });
        console.log('[PROXY] enabled replace ', file.file);
    },
    removeReplaceFile: function(file) {
        var indexes = [];
        replaces.forEach(function(item, i) {
            if (item.patternStr == file.pattern) {
                indexes.push(i);
            }
        });
        ...
        console.log('[PROXY] disabled replace ', file.file);
    }
};
